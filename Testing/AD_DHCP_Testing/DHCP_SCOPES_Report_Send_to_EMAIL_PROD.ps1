...
Email -AttachSelf -AttachSelfName 'DHCP Report' {
    EmailHeader {
        EmailFrom -Address 'DNS_DHCP_SCOPES_Report@igt.com' # Made up name to send as, change as needed
        EmailTo -Addresses "Stephen.McKee@igt.com" # Where you what to send the email to
        EmailServer -Server 'smtp.igt.com' -Port 25  #- change if needed
        EmailOptions -Priority High -DeliveryNotifications Never
        EmailSubject -Subject 'Test-MYIGT-DHCP Report - Scope Utilization'
    }
    EmailBody {
